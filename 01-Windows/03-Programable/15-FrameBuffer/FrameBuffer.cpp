...
void Initialize(void)
{
	...
	//Bind FrameBuffer with fbo
	glBindFramebuffer(GL_FRAMEBUFFER, fbo);

		//Create Texture
		glGenTextures(1, &texture);

		//Bind it with texture
		glBindTexture(GL_TEXTURE_2D, texture);
			...
			{
				fprintf(gpFile_rvg, "FrameBuffer Successfully\n");
			}
			else
			{
				fprintf(gpFile_rvg, "FrameBuffer Failed\n");
			}
...
void Display(void)
{
	...
	translateMatrix = vmath::translate(-1.3f, -0.3f, -3.5f);
	...
	rotationMatrix = vmath::rotate(CAngle_rvg, 0.0f, 0.0f, 1.0f) * vmath::rotate(CAngle_rvg, 0.0f, 1.0f, 0.0f) * vmath::rotate(CAngle_rvg, 1.0f, 0.0f, 0.0f);
