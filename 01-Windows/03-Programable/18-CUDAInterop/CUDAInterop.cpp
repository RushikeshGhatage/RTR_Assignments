...
#define MYARRAY_SIZE MESH_WIDTH * MESH_HEIGHT * 4
...
float position_rvg[MESH_WIDTH][MESH_HEIGHT][4];
...
GLuint vbo_cpu_rvg;		//Vertex Buffer Objects
GLuint vbo_gpu_rvg;		//Vertex Buffer Objects
GLuint mvpMatrixUniform;

unsigned int pressedDigit_rvg = 0;

mat4 perspectiveProjectionMatrix;

cudaError_t cudaResult_rvg;
...
void ToggleFullScreen(void)
{
	...
	else
	{
		...
		SetWindowPos(ghwnd_rvg, HWND_TOP, 0, 0, 0, 0, SWP_NOMOVE | SWP_NOSIZE | SWP_NOZORDER | SWP_NOOWNERZORDER | SWP_FRAMECHANGED);
		ShowCursor(TRUE);
...
void Initialize(void)
{
	...
	//Code
	cudaResult_rvg = cudaGetDeviceCount(&devCount_rvg);

	if (cudaResult_rvg != cudaSuccess)
	{
		fprintf(gpFile_rvg, "Unable to Get Device Count!");
		DestroyWindow(ghwnd_rvg);
	}
	else if(devCount_rvg == 0)
	{
		fprintf(gpFile_rvg, "Device Count is Zero!");
		DestroyWindow(ghwnd_rvg);
	}
	else
	{
	...
	mvpMatrixUniform = glGetUniformLocation(gShaderProgramObject_rvg, "u_mvpMatrix");
	...
	glBindVertexArray(vao_rvg);
			...
			glGenBuffers(1, &vbo_cpu_rvg);
 
			//Bind Buffer Array
			glBindBuffer(GL_ARRAY_BUFFER, vbo_cpu_rvg);

			//Fill Data into Buffer Array
			glBufferData(GL_ARRAY_BUFFER, (MYARRAY_SIZE * sizeof(float)), NULL, GL_DYNAMIC_DRAW);

			//Unbind Buffer Array
			glBindBuffer(GL_ARRAY_BUFFER, 0);

			//Generate Buffer Array
			glGenBuffers(1, &vbo_gpu_rvg);
 
			//Bind Buffer Array
			glBindBuffer(GL_ARRAY_BUFFER, vbo_gpu_rvg);
			...
			cudaResult_rvg = cudaGraphicsGLRegisterBuffer(&cuda_graphics_resource, vbo_gpu_rvg, cudaGraphicsMapFlagsWriteDiscard);
			if (cudaResult_rvg != cudaSuccess)
			{
				fprintf(gpFile_rvg, "cudaGraphicsGLRegisterBuffer is Failed!");
				DestroyWindow(ghwnd_rvg);
			}
...
void Display(void)
{
	...
	glBindVertexArray(vao_rvg);
			...
			if (bOnGPU == true)
			{
				cudaResult_rvg = cudaGraphicsMapResources(1, &cuda_graphics_resource, 0);
				if (cudaResult_rvg != cudaSuccess)
				{
					fprintf(gpFile_rvg, "cudaGraphicsMapResources is Failed!");
					DestroyWindow(ghwnd_rvg);
				}

				float4 *pPos_rvg = NULL;
				size_t numBytes;

				cudaResult_rvg = cudaGraphicsResourceGetMappedPointer((void **)&pPos_rvg, &numBytes, cuda_graphics_resource);
				if (cudaResult_rvg != cudaSuccess)
				{
					fprintf(gpFile_rvg, "cudaGraphicsResourceGetMappedPointer is Failed!");
					DestroyWindow(ghwnd_rvg);
				}

				launchCUDAKernel(pPos_rvg, MESH_WIDTH, MESH_HEIGHT, animationTime);

				cudaResult_rvg = cudaGraphicsUnmapResources(1, &cuda_graphics_resource, 0);
				if (cudaResult_rvg != cudaSuccess)
				{
					fprintf(gpFile_rvg, "cudaGraphicsUnmapResources is Failed!");
					DestroyWindow(ghwnd_rvg);
				}
				...
				//Fill Data into Buffer Array
				//glBufferData(GL_ARRAY_BUFFER, MYARRAY_SIZE * sizeof(float), position_rvg, GL_DYNAMIC_DRAW);
		...
		glVertexAttribPointer(RVG_ATTRIBUTE_POSITION, 4, GL_FLOAT, GL_FALSE, 0, NULL);
		...
		if (pressedDigit_rvg == 0)
		{
			glVertexAttrib3f(RVG_ATTRIBUTE_COLOR, 1.0f, 0.0f, 0.0f);
		}

		else if (pressedDigit_rvg == 1)
		{
			glVertexAttrib3f(RVG_ATTRIBUTE_COLOR, 0.0f, 1.0f, 0.0f);
		}
	...
	for (int i = 0; i < MESH_WIDTH; i++)
	{
		for (int j = 0; j < MESH_HEIGHT; j++)
		{
			for (int k = 0; k < 4; k++)
			{
				...
				if (k == 0)
				{
					position_rvg[i][j][k] = u;
				}

				if (k == 1)
				{
					position_rvg[i][j][k] = w;
				}

				if (k == 2)
				{
					position_rvg[i][j][k] = v;
				}

				if (k == 3)
				{
					position_rvg[i][j][k] = 1.0f;
				}
			}
		}
	}
...
void Uninitialize()
{
	...
	if (vbo_cpu_rvg)
	{
		glDeleteBuffers(1, &vbo_cpu_rvg);
		vbo_cpu_rvg = 0;
	}

	if (vbo_gpu_rvg)
	{
		glDeleteBuffers(1, &vbo_gpu_rvg);
		vbo_gpu_rvg = 0;
	}
