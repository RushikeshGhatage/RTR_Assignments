...
void Initialize(void)
{
	...
	//Compile Shader
	glCompileShader(gGeometryShaderObject_rvg);	
	...
	glGetShaderiv(gGeometryShaderObject_rvg, GL_COMPILE_STATUS, &shaderCompilationStatus);
	if (shaderCompilationStatus == GL_FALSE)
	{
		glGetShaderiv(gGeometryShaderObject_rvg, GL_INFO_LOG_LENGTH, &infoLogLength);
		if (infoLogLength > 0)
		{
			szBuffer = (GLchar *)malloc(infoLogLength);
			if (szBuffer != NULL)
			{
				GLsizei written;
				glGetShaderInfoLog(gGeometryShaderObject_rvg, infoLogLength, &written, szBuffer);
