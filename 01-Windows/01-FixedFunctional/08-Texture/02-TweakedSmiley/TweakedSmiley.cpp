...
GLuint Smiley_Texture_rvg;
unsigned int PressedDigit_rvg = 1;
...
void Initialize(void)
{
	...
	//Load Texture
	loadGLTexture(&Smiley_Texture_rvg, MAKEINTRESOURCE(SMILEY_BITMAP));
...
void Display(void)
{
	...
	glBindTexture(GL_TEXTURE_2D, Smiley_Texture_rvg);
...
void Uninitialize()
{
	...
	if (Smiley_Texture_rvg)
	{
		glDeleteTextures(1, &Smiley_Texture_rvg);
	}
