...
#define MOKEYHEAD_Y_TRANSLATE	-7.0f
#define MOKEYHEAD_Z_TRANSLATE	-30.0f
...
void LoadMeshData(void)
{
	...
	g_fp_meshFile = fopen("dragon.OBJ", "r");
	if (!g_fp_meshFile)
	{
		Uninitialize();
	}
	...
	fprintf(gpFile_rvg, "gp_vertices->size:%zu gp_texture->size:%zu gp_normals->size:%zu gp_face_tri->size:%zu\n",
...
void Display(void)
{
	...
	for (int i = 0; i != gp_face_tri->size; ++i)
	{
		...
		for (int j = 0; j != NR_TRIANGLE_VERTICES; j++)
		{
			...
			int ni = gp_face_normals->pp_arr[i][j] - 1;
			glNormal3f(gp_normals->pp_arr[ni][0], gp_normals->pp_arr[ni][1], gp_normals->pp_arr[ni][2]);
