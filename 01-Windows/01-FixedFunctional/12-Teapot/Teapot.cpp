...
{	
	...
	for (int i_rvg = 0; i_rvg < sizeof(face_indicies) / sizeof(face_indicies[0]); i_rvg++)
	{
		for (int j_rvg = 0; j_rvg < 3; j_rvg++)
		{
			int vi_rvg = face_indicies[i_rvg][j_rvg];
			int ni_rvg = face_indicies[i_rvg][j_rvg + 3];
			int ti_rvg = face_indicies[i_rvg][j_rvg + 6];

			glNormal3f(normals[ni_rvg][0], normals[ni_rvg][1], normals[ni_rvg][2]);
			glTexCoord2f(textures[ti_rvg][0], textures[ti_rvg][1]);
			glVertex3f(vertices[vi_rvg][0], vertices[vi_rvg][1], vertices[vi_rvg][2]);
		}
	}
	glEnd();
