...
#define WIN_NAME "DirectX : Rotating Colored Triangle Without Culling"
...
ID3D11Buffer *gpID3D11Buffer_VertexBuffer_Position_rvg = NULL;
ID3D11Buffer *gpID3D11Buffer_VertexBuffer_Color_rvg = NULL;
...
HRESULT initialize(void)
{
	...
	hr_rvg = gpID3D11Device_rvg->CreateBuffer(&bufferDesc_rvg,
												NULL,
												&gpID3D11Buffer_VertexBuffer_Position_rvg);
	...
	gpID3D11DeviceContext_rvg->Map(gpID3D11Buffer_VertexBuffer_Position_rvg,
									0,
	...
	gpID3D11DeviceContext_rvg->Unmap(gpID3D11Buffer_VertexBuffer_Position_rvg, 0);
	...
	//Initialize and Create Vertex Buffer
	ZeroMemory((void *)&bufferDesc_rvg, sizeof(D3D11_BUFFER_DESC));
	bufferDesc_rvg.ByteWidth = sizeof(float) * ARRAYSIZE(colors);
	bufferDesc_rvg.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	bufferDesc_rvg.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
	bufferDesc_rvg.Usage = D3D11_USAGE_DYNAMIC;
	...
	hr_rvg = gpID3D11Device_rvg->CreateBuffer(&bufferDesc_rvg,
												NULL,
												&gpID3D11Buffer_VertexBuffer_Color_rvg);
	...
	//Copy Vertices into Buffer
	ZeroMemory((void *)&mappedSubresources_rvg, sizeof(D3D11_MAPPED_SUBRESOURCE));
	gpID3D11DeviceContext_rvg->Map(gpID3D11Buffer_VertexBuffer_Color_rvg,
									0,
									D3D11_MAP_WRITE_DISCARD,
									0,
									&mappedSubresources_rvg);

	memcpy(mappedSubresources_rvg.pData, colors, sizeof(colors));
	gpID3D11DeviceContext_rvg->Unmap(gpID3D11Buffer_VertexBuffer_Color_rvg, 0);
...
void display(void)
{
	...
	gpID3D11DeviceContext_rvg->IASetVertexBuffers(0, 1, &gpID3D11Buffer_VertexBuffer_Position_rvg, &stride_rvg, &offset_rvg);

	stride_rvg = sizeof(float) * 3;
	offset_rvg = 0;
	gpID3D11DeviceContext_rvg->IASetVertexBuffers(1, 1, &gpID3D11Buffer_VertexBuffer_Color_rvg, &stride_rvg, &offset_rvg);
	...
	translationMatrix_rvg = XMMatrixTranslation(0.0f, 0.0f, 4.0f);
	rotationMatrix_rvg = XMMatrixRotationY(TAngle_rvg);
...
void uninitialize(void)
{
	...
	if (gpID3D11Buffer_VertexBuffer_Position_rvg)
	{
		gpID3D11Buffer_VertexBuffer_Position_rvg->Release();
		gpID3D11Buffer_VertexBuffer_Position_rvg = NULL;
	}

	if (gpID3D11Buffer_VertexBuffer_Color_rvg)
	{
		gpID3D11Buffer_VertexBuffer_Color_rvg->Release();
		gpID3D11Buffer_VertexBuffer_Color_rvg = NULL;
	}	
