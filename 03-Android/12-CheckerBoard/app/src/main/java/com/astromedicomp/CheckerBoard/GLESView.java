...
public class GLESView extends GLSurfaceView implements GLSurfaceView.Renderer, OnGestureListener, OnDoubleTapListener
{
	...
	private final int CheckImageHeight_rvg = 64;
	private final int CheckImageWidth_rvg = 64;

	private byte checkImage[] = new byte[CheckImageHeight_rvg * CheckImageWidth_rvg * 4];
	...
	private int[] CheckerBoard_rvg = new int[1];
	...
	private void Initialize(GL10 gl)
	{
		...
		textureSamplerUniform = GLES32.glGetUniformLocation(gShaderProgramObject_rvg, "u_texture_sampler");
		...
		//Set Texture
		CheckerBoard_rvg[0] = loadGLTexture();
	...
	private void makeCheckImage()
	{
		...
		//Code
		for (i = 0; i < CheckImageHeight_rvg; i++)
		{
			for (j = 0; j < CheckImageWidth_rvg; j++)
			{
				c = ((i & 8) ^ (j & 8)) * 255;
	...
	private int loadGLTexture()
	{
		...
		//Local Variable Declaration
		Bitmap bitmap = Bitmap.createBitmap(CheckImageWidth_rvg, CheckImageHeight_rvg, Bitmap.Config.ARGB_8888);

		//Fill Data into Buffer Array
		ByteBuffer byteBuffer_rvg = ByteBuffer.allocateDirect(CheckImageWidth_rvg * CheckImageHeight_rvg * 4);
		byteBuffer_rvg.order(ByteOrder.nativeOrder());
		byteBuffer_rvg.put(checkImage);
		byteBuffer_rvg.position(0);

		bitmap.copyPixelsFromBuffer(byteBuffer_rvg);
	...
	private void Display()
	{
		...
		GLES32.glBindTexture(GLES32.GL_TEXTURE_2D, CheckerBoard_rvg[0]);
		...
		GLES32.glBindVertexArray(vao_plane_rvg[0]);
				...
				GLES32.glBindBuffer(GLES32.GL_ARRAY_BUFFER, vbo_plane_position_rvg[0]);
					...
					ByteBuffer byteBuffer_rvg = ByteBuffer.allocateDirect(straightQuad.length * 4);
					byteBuffer_rvg.order(ByteOrder.nativeOrder());
					FloatBuffer floatBuffer_rvg = byteBuffer_rvg.asFloatBuffer();
					floatBuffer_rvg.put(straightQuad);
					floatBuffer_rvg.position(0);

					GLES32.glBufferData(GLES32.GL_ARRAY_BUFFER, (straightQuad.length * 4), floatBuffer_rvg, GLES32.GL_DYNAMIC_DRAW);
				...
				GLES32.glBindBuffer(GLES32.GL_ARRAY_BUFFER, vbo_plane_texcoord_rvg[0]);
					...
					ByteBuffer byteBufferQuad = ByteBuffer.allocateDirect(texCoord.length * 4);
					byteBufferQuad.order(ByteOrder.nativeOrder());
					FloatBuffer textBuffer_rvg = byteBufferQuad.asFloatBuffer();
					textBuffer_rvg.put(texCoord);
					textBuffer_rvg.position(0);

					GLES32.glBufferData(GLES32.GL_ARRAY_BUFFER, (4 * 2 * 4), textBuffer_rvg, GLES32.GL_DYNAMIC_DRAW);
				...
				straightQuad[7] = 1.0f;
				straightQuad[8] = -1.41421f;

				straightQuad[9] = 2.41421f;
				straightQuad[10] = -1.0f;
				straightQuad[11] = -1.41421f;

				/////////////////////////////// FOR VERTEX ///////////////////////////////
				GLES32.glBindBuffer(GLES32.GL_ARRAY_BUFFER, vbo_plane_position_rvg[0]);

					byteBuffer_rvg = ByteBuffer.allocateDirect(straightQuad.length * 4);
					byteBuffer_rvg.order(ByteOrder.nativeOrder());
					floatBuffer_rvg = byteBuffer_rvg.asFloatBuffer();
					floatBuffer_rvg.put(straightQuad);
					floatBuffer_rvg.position(0);
					GLES32.glBufferData(GLES32.GL_ARRAY_BUFFER, (4 * 3 * 4), floatBuffer_rvg, GLES32.GL_DYNAMIC_DRAW);
