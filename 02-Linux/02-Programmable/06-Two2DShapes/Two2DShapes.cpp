...
GLuint vbo_position_triangle_rvg;			//for Vertex
GLuint vbo_color_triangle_rvg;				//for Color

GLuint vao_square_rvg;					//Vertex Array Objects
GLuint vbo_position_square_rvg;			//for Vertex
GLuint vbo_color_square_rvg;				//for Color
...
void CreateWindow(void)
{
	...
	XStoreName(gpDisplay_rvg, gWindow_rvg, "Rushikesh Vinod Ghatage : Two 2D Shapes");
...
void Initialize(void)
{
	...
	const GLfloat triangleColors_rvg[] = 
	{
		1.0f, 0.0f, 0.0f,		//Red
	...
	const GLfloat squareVertices_rvg[] = 
	{
	...
	glBindVertexArray(vao_triangle_rvg);
			...
			glGenBuffers(1, &vbo_position_triangle_rvg);
 
			//Bind Buffer Array
			glBindBuffer(GL_ARRAY_BUFFER, vbo_position_triangle_rvg);
			...
			/////////////////////////////// FOR COLOR ///////////////////////////////
			//Generate Buffer Array
 			glGenBuffers(1, &vbo_color_triangle_rvg);

			//Bind Buffer Array
			glBindBuffer(GL_ARRAY_BUFFER, vbo_color_triangle_rvg);

			//Fill Data into Buffer Array
			glBufferData(GL_ARRAY_BUFFER, sizeof(triangleColors_rvg), triangleColors_rvg, GL_STATIC_DRAW);
	...
	glBindVertexArray(vao_square_rvg);
			/////////////////////////////// FOR VERTEX ///////////////////////////////
			//Generate Buffer Array
			glGenBuffers(1, &vbo_position_square_rvg);
 
			//Bind Buffer Array
			glBindBuffer(GL_ARRAY_BUFFER, vbo_position_square_rvg);

			//Fill Data into Buffer Array
			glBufferData(GL_ARRAY_BUFFER, sizeof(squareVertices_rvg), squareVertices_rvg, GL_STATIC_DRAW);
...
void Uninitialize(void)
{
	...
	if (vbo_position_triangle_rvg)
	{
		glDeleteBuffers(1, &vbo_position_triangle_rvg);
		vbo_position_triangle_rvg = 0;
	}

	if (vbo_color_triangle_rvg)
	{
		glDeleteBuffers(1, &vbo_color_triangle_rvg);
		vbo_color_triangle_rvg = 0;
	}
	...
	if (vbo_position_square_rvg)
	{
		glDeleteBuffers(1, &vbo_position_square_rvg);
		vbo_position_square_rvg = 0;
	}

	if (vbo_color_square_rvg)
	{
		glDeleteBuffers(1, &vbo_color_square_rvg);
		vbo_color_square_rvg = 0;
	}
