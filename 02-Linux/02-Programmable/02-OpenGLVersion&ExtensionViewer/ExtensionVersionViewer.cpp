...
void CreateWindow(void)
{
	...
	XStoreName(gpDisplay_rvg, gWindow_rvg, "Rushikesh Vinod Ghatage : OpenGL Extension Version Viewer");
...
void Initialize(void)
{
	...
	//OpenGL related Log
	fprintf(gpFile_rvg, "\nOpenGL Related Log\n\n");
	fprintf(gpFile_rvg, "OpenGL Vendor : %s\n", glGetString(GL_VENDOR));
	fprintf(gpFile_rvg, "OpenGL Renderer : %s\n",glGetString(GL_RENDERER));
	fprintf(gpFile_rvg, "OpenGL Version : %s\n", glGetString(GL_VERSION));
	fprintf(gpFile_rvg, "GLSL (Graphic Library Shading Language) Version : %s\n", glGetString(GL_SHADING_LANGUAGE_VERSION));

	//OpenGL Enabled Extensions
	fprintf(gpFile_rvg, "\nOpenGL Enabled Extensions\n\n");
	glGetIntegerv(GL_NUM_EXTENSIONS, &numExt_rvg);
	for (int i = 0; i < numExt_rvg; i++)
	{
		fprintf(gpFile_rvg, "%s\n", glGetStringi(GL_EXTENSIONS, i));
	}
