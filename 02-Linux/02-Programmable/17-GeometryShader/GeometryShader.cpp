...
GLuint gGeometryShaderObject_rvg;
...
void CreateWindow(void)
{
	...
	XStoreName(gpDisplay_rvg, gWindow_rvg, "Rushikesh Vinod Ghatage : Geometry Shader");
...
void Initialize(void)
{
	...
	// ****************** Geometry Shader ******************
	//Create Shader
	gGeometryShaderObject_rvg = glCreateShader(GL_GEOMETRY_SHADER);
	...
	//Feed to Shader Object
	glShaderSource(gGeometryShaderObject_rvg, 1, (const GLchar **) &GeometryShaderCode, NULL);

	//Compile Shader
	glCompileShader(gGeometryShaderObject_rvg);	//Here there should be Shader Compilation Error Checking
	...
	glGetShaderiv(gGeometryShaderObject_rvg, GL_COMPILE_STATUS, &shaderCompilationStatus_rvg);
	if (shaderCompilationStatus_rvg == GL_FALSE)
	{
		glGetShaderiv(gGeometryShaderObject_rvg, GL_INFO_LOG_LENGTH, &infoLogLength_rvg);
		if (infoLogLength_rvg > 0)
		{
			szBuffer_rvg = (GLchar *)malloc(infoLogLength_rvg);
			if (szBuffer_rvg != NULL)
			{
				GLsizei written_rvg;
				glGetShaderInfoLog(gGeometryShaderObject_rvg, infoLogLength_rvg, &written_rvg, szBuffer_rvg);
				fprintf(gpFile_rvg, "Geometry Shader Compilation Log : %s\n", szBuffer_rvg);
				free(szBuffer_rvg);
				Uninitialize();
			}
		}
	...
	{
		fprintf(gpFile_rvg, "Geometry Shader Compilation is done Successfully.\n");
	}	
	...
	glAttachShader(gShaderProgramObject_rvg, gGeometryShaderObject_rvg);	//Geometry Shader	
