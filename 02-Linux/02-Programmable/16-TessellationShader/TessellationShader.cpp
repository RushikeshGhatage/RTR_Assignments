...
void CreateWindow(void)
{
	...
	XStoreName(gpDisplay_rvg, gWindow_rvg, "Rushikesh Vinod Ghatage : Tessellation Shader");
...
void Initialize(void)
{
	...
	//Compile Shader
	glCompileShader(gTessellationControlShaderObject_rvg);	//Here there should be Shader Compilation Error Checking
	...
	glGetShaderiv(gTessellationControlShaderObject_rvg, GL_COMPILE_STATUS, &shaderCompilationStatus_rvg);
	if (shaderCompilationStatus_rvg == GL_FALSE)
	{
		glGetShaderiv(gTessellationControlShaderObject_rvg, GL_INFO_LOG_LENGTH, &infoLogLength_rvg);
		if (infoLogLength_rvg > 0)
		{
			szBuffer_rvg = (GLchar *)malloc(infoLogLength_rvg);
			if (szBuffer_rvg != NULL)
			{
				GLsizei written_rvg;
				glGetShaderInfoLog(gTessellationControlShaderObject_rvg, infoLogLength_rvg, &written_rvg, szBuffer_rvg);
	...
	{
		fprintf(gpFile_rvg, "Tessellation Control Compilation is done Successfully.\n");
	}
	...
	//Compile Shader
	glCompileShader(gTessellationEvaluationShaderObject_rvg);	//Here there should be Shader Compilation Error Checking
	...
	glGetShaderiv(gTessellationEvaluationShaderObject_rvg, GL_COMPILE_STATUS, &shaderCompilationStatus_rvg);
	if (shaderCompilationStatus_rvg == GL_FALSE)
	{
		glGetShaderiv(gTessellationEvaluationShaderObject_rvg, GL_INFO_LOG_LENGTH, &infoLogLength_rvg);
		if (infoLogLength_rvg > 0)
		{
			szBuffer_rvg = (GLchar *)malloc(infoLogLength_rvg);
			if (szBuffer_rvg != NULL)
			{
				GLsizei written_rvg;
				glGetShaderInfoLog(gTessellationEvaluationShaderObject_rvg, infoLogLength_rvg, &written_rvg, szBuffer_rvg);
	...
	{
		fprintf(gpFile_rvg, "Tessellation Evaluation Compilation is done Successfully.\n");
	}	
