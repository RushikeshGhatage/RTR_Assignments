...
void pushMatrix_rvg(mat4);
mat4 popMatrix_rvg(void);
mat4 stack[MAX];
int top_rvg = -1;
...
void CreateWindow(void)
{
	...
	XStoreName(gpDisplay_rvg, gWindow_rvg, "Rushikesh Vinod Ghatage : Perspective Projection Sphere");
...
void Draw(void)
{
	...
	pushMatrix_rvg(modelViewProjectionMatrix);
		...
		pushMatrix_rvg(modelViewProjectionMatrix);
			...
			glBindVertexArray(0);
		modelViewProjectionMatrix = popMatrix_rvg();
...
void pushMatrix_rvg(mat4 mat4by4)
{
	//Code
	if (top_rvg >= MAX - 1)
	{
